...
class InvertibleConv1x1LU(nn.Module):
    ...
    def __init__(self, input_dim, device=None, dtype=None):
        ...
        W_init = torch.linalg.qr(
            torch.randn(input_dim, input_dim, dtype=dtype, device=device)
        )[0]
    ...
    def _W(self):
        ...
        U_full = self.U * self.u_mask + torch.diag(
...
class SplineCoupling1x1Flow(nn.Module):
    ...
    def __init__(self, depth, input_dim, device, dtype, seed,
                 init_scale=1e-3, num_bins=8, B=3.0):
        super().__init__()
        self.depth = depth
        self.input_dim = input_dim
        self.device = device
        ...
        self.conv1x1s = nn.ModuleList([
            InvertibleConv1x1LU(input_dim, device=device, dtype=dtype)
            for _ in range(depth)
        ])
