...
Drop-in replacement for :class:`src.flows.flows.CouplingFlow` that uses
...
class SplineCouplingLayer(nn.Module):
    ...
    def __init__(self, input_dim, num_bins=8, B=3.0, device=None, dtype=None,
                 init_scale=1e-3, hidden_dim=None):
        super().__init__()
    ...
    def _spline_params(self, context):
        ...
        ones = torch.ones(*inner.shape[:-1], 1,
    ...
    def forward(self, a):
        ...
        out_flat, ld_flat = rq_spline_forward(
            z2_flat, w_flat, h_flat, d_flat, self.B, inverse=False,
        )
...
class SplineCouplingFlow(nn.Module):
    ...
    Signature matches :class:`src.flows.flows.CouplingFlow` (plus optional
    ``num_bins`` / ``B``). Returns ``(out, -LDJ)``.
    """

    def __init__(self, depth, input_dim, device, dtype, seed,
                 init_scale=1e-3, num_bins=8, B=3.0, hidden_dim=None):
        super().__init__()
        self.depth = depth
        self.input_dim = input_dim
        self.device = device
        ...
        self.biyections = nn.ModuleList([
            SplineCouplingLayer(
                input_dim=input_dim, num_bins=num_bins, B=B,
                device=device, dtype=dtype, init_scale=init_scale,
