...
class FTIP(torch.nn.Module):
    ...
    def load_state_dict(self, state_dict, **kwargs):
        ...
        if has_affine and self.flow.affine is None:
            ...
            self.flow.affine = AffineLayer(
                self.flow.input_dim, self.flow.device, self.flow.dtype,
                learnable=True,
            )
    ...
    def KL(self):
        ...
        if not hasattr(self, "_eps_flat"): raise RuntimeError("Call predict_f before KL")
    ...
    def warm_start_from_vip(self, vip_model, learnable_affine=False):
        ...
        self.generative_function.load_state_dict(
            vip_model.generative_function.state_dict()
        )
        for dst, src in zip(self.generative_function.modules(),
                            vip_model.generative_function.modules()):
...
class UnifiedFTIP(torch.nn.Module):
    ...
    def __init__(
        ...
        if likelihood not in ("regression", "binary", "multiclass"):
            raise ValueError(f"likelihood must be 'regression', 'binary', or 'multiclass', got '{likelihood}'")
        if likelihood == "multiclass" and num_classes is None:
            raise ValueError("num_classes is required for multiclass likelihood")
        ...
        self.q_mu = torch.nn.Parameter(
            torch.zeros(S, output_dim, dtype=dtype, device=device)
        )
    ...
    def _nelbo_gaussian(self, X, y):
        ...
        if self.likelihood_type == "regression":
            ve = gaussian_variational_expectations(
                ...
                self.bb_alpha, self.log_variance,
            )
        else:
    ...
    def fit(self, train_loader, warmup_epochs=None, warmup_lr=1e-3,
            flow_epochs=None, flow_lr=1e-4, cosine_annealing=True,
            use_tqdm=False, return_loss=False):
        ...
        if warmup_epochs and warmup_epochs > 0:
            ...
            self.train()
            loop = tqdm(range(warmup_epochs), unit=" epoch", desc="Warmup") if use_tqdm else range(warmup_epochs)
            for _ in loop:
                for inputs, target in train_loader:
                    inputs = inputs.to(self.device)
                    target = target.to(self.device)
        ...
        if flow_epochs and flow_epochs > 0:
            ...
            self.train()
            loop = tqdm(range(flow_epochs), unit=" epoch", desc="Flow") if use_tqdm else range(flow_epochs)
            for _ in loop:
                for inputs, target in train_loader:
                    inputs = inputs.to(self.device)
                    target = target.to(self.device)
