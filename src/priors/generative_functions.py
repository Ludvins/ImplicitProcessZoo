...
from ..utils.quadrature import hermgauss, hermgaussquadrature
...
class BayesLinear(GenerativeFunction):
    def __init__(
        ...
        self.weight_log_sigma = torch.nn.Parameter(
            torch.full([input_dim, output_dim], weight_log_sigma_init,
                       dtype=dtype, device=device)
        )
        self.bias_log_sigma = torch.nn.Parameter(
            torch.full([1, output_dim], weight_log_sigma_init,
                       dtype=dtype, device=device)
        )
...
class BayesConv2d(GenerativeFunction):
    ...
    def __init__(
        ...
        self.weight_log_sigma = torch.nn.Parameter(
            torch.full((out_channels, in_channels, *ksz), weight_log_sigma_init,
                       dtype=dtype, device=device)
        )
        ...
        self.bias_log_sigma = torch.nn.Parameter(
            torch.full((out_channels,), weight_log_sigma_init,
                       dtype=dtype, device=device)
        )
    ...
    def forward(self, x):
        ...
        if x.dim() != 5:
            raise RuntimeError(
                f"BayesConv2d expects (S, N, C, H, W); got shape {tuple(x.shape)}"
            )
        S, N, C_in, H, W = x.shape
        if S != self.num_samples or C_in != self.in_channels:
            raise RuntimeError(
        ...
        y_g = torch.nn.functional.conv2d(
            x_g, w_g, b_g,
            stride=self.stride, padding=self.padding, groups=S,
        )
        H_o, W_o = y_g.shape[-2:]
        # Reshape back to (S, N, C_out, H', W')
        return (
            y_g.reshape(N, S, self.out_channels, H_o, W_o)
               .permute(1, 0, 2, 3, 4)
               .contiguous()
        )
...
class BayesianCNNFull(GenerativeFunction):
    ...
    def __init__(
        ...
        self.conv1 = BayesConv2d(
            num_samples, in_channels, 6, kernel_size=5, padding=2,
            device=device, fix_random_noise=fix_random_noise,
            weight_log_sigma_init=weight_log_sigma_init,
            seed=seed, dtype=dtype,
        )
        self.conv2 = BayesConv2d(
            num_samples, 6, 16, kernel_size=5, padding=0,
            device=device, fix_random_noise=fix_random_noise,
            weight_log_sigma_init=weight_log_sigma_init,
    ...
    def KL(self):
        return (self.conv1.KL() + self.conv2.KL()
...
class BayesianResNet(GenerativeFunction):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            device=device, fix_random_noise=fix_random_noise, seed=seed, dtype=dtype,
        )
        if input_dim not in self._SHAPE_MAP:
            raise ValueError(
