...
class MetricsClassification:
    ...
    def update(self, y, loss, mean_pred, std_pred=None, light=True):
        ...
        self.nll += torch.nn.functional.cross_entropy(
...
class MetricsBinary:
    ...
    def update(self, y, loss, mean_pred, std_pred=None, light=True):
        ...
        self.nll += -(
            y_flat * torch.log(p) + (1 - y_flat) * torch.log(1 - p)
        ).sum()
    ...
    def _compute_ece(self, p, y):
        ...
        bin_boundaries = torch.linspace(
            0.0, 1.0, self.n_bins + 1, device=p.device
        )
