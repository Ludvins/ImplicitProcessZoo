...
class SSGE:
    ...
    def compute_score_nystrom(self, X_ref, X_eval):
        ...
        assert X_ref.shape[-1] == X_eval.shape[-1], (
            f"Dimension mismatch: X_ref[-1]={X_ref.shape[-1]}, "
            f"X_eval[-1]={X_eval.shape[-1]}"
        )
        ...
        # Nugget-regularized eigendecomposition
        K_reg = K_ref + self.nugget * torch.eye(
            S_ref, dtype=K_ref.dtype, device=K_ref.device
        )
        eigenvalues, eigenvectors = torch.linalg.eigh(K_reg)
        eigenvalues = eigenvalues[-J:]
        eigenvectors = eigenvectors[:, -J:]
...
class FBNN(torch.nn.Module):
    ...
    def nelbo(self, X_batch, y):
        ...
        if self.likelihood_type == "multiclass":
            assert F_batch.shape[-1] == self.num_classes, (
                f"Expected {self.num_classes} classes in last dim, "
                f"got {F_batch.shape[-1]}"
            )
    ...
    def _sample_measurement_set(self, X_batch):
        ...
        if self.num_context > 0:
            X_context = torch.randn(
                self.num_context, D_in,
        ...
        if self._reservoir is not None and self.num_measurement > 0:
            ...
            X_train = self._reservoir[idx].to(
                dtype=X_batch.dtype, device=X_batch.device
            )
    ...
    def fit(self, train_loader, optimizer=None, lr=0.001, epochs=None,
        ...
        """
        # Build measurement reservoir (O(reservoir_size) memory)
        ...
        # Derive device from model parameters (tracks .to() calls)
        device = self.y_mean.device
