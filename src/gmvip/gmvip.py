...
class GeneralizedMatheronVIP(nn.Module):
    ...
    def __init__(
        ...
        if likelihood_type != "gaussian":
            raise NotImplementedError("Only Gaussian scalar regression is supported.")
        self.base_prior = base_prior
        self.operator_type = str(operator_type)
        self.posterior_type = str(posterior_type)
        self.likelihood_type = str(likelihood_type)
        ...
        if self.operator_type == "empirical":
            ...
            if inducing_scale != "prior_cholesky":
                raise ValueError("empirical operator only supports inducing_scale='prior_cholesky'.")
    ...
    def inducing_values_from_coefficients(self, coefficients: torch.Tensor) -> torch.Tensor:
        return self.operator.whitened_to_inducing(coefficients.to(dtype=self.Z.dtype, device=self.Z.device))
    ...
    def compute_cross_covariance(self, X: torch.Tensor) -> torch.Tensor:
        if not hasattr(self.operator, "compute_cross_covariance"):
            raise AttributeError("compute_cross_covariance is only available for empirical operator.")
    ...
    def sample_posterior_values_with_kl(
        ...
        if self.posterior_type not in {"gaussian", "realnvp"}:
            raise NotImplementedError("sample_posterior_values_with_kl requires gaussian or realnvp q(a).")
        generator = self._make_generator(seed)
    ...
    def elbo(
        ...
        return self._elbo_gaussian(X_batch, y_batch, num_samples, num_data, beta, seed=seed, data_alpha=data_alpha)
    ...
    def _base_metrics(
        ...
        data_alpha_tensor = torch.tensor(float(data_alpha), dtype=self.Z.dtype, device=self.Z.device)
        return {
            "elbo": elbo.detach(),
    ...
    def forward_prior(self, X: torch.Tensor, num_samples: int) -> torch.Tensor:
        samples = self.sample_prior_values(X, int(num_samples)).unsqueeze(-1)
    ...
    def _train_step(self, optimizer: torch.optim.Optimizer, X: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        X = self._as_model_input(X)
        y = self._as_target(y)
        self._step += 1
