...
class TestFTIPConstruction:
    ...
    def test_regression(self, bnn, coupling_flow):
        model = FTIP(bnn, NUM_SAMPLES, OUTPUT_DIM, coupling_flow,
                       "regression", NUM_DATA, NUM_SAMPLES,
    ...
    def test_multiclass(self, bnn_multiclass):
        ...
        model = FTIP(bnn_multiclass, NUM_SAMPLES, 3, flow,
                       "multiclass", NUM_DATA, NUM_SAMPLES,
    ...
    def test_invalid_likelihood(self, bnn, coupling_flow):
        with pytest.raises(ValueError):
            FTIP(bnn, NUM_SAMPLES, OUTPUT_DIM, coupling_flow,
                  "poisson", NUM_DATA, NUM_SAMPLES,
...
class TestFTIPShapes:
    ...
    def model(self, bnn, coupling_flow):
        return FTIP(bnn, NUM_SAMPLES, OUTPUT_DIM, coupling_flow,
                      "regression", NUM_DATA, NUM_SAMPLES,
                      device=DEVICE, dtype=DTYPE, seed=SEED)
...
class TestFTIPCNNWarmStart:
    ...
    def test_warm_start_from_vip(self, cnn_multiclass, mnist_multiclass_data):
        ...
        vip = VIP(cnn_multiclass, NUM_SAMPLES, NUM_CLASSES, "multiclass", NUM_DATA,
                  num_classes=NUM_CLASSES, device=DEVICE, dtype=DTYPE, seed=SEED)
        opt = torch.optim.Adam(vip.parameters(), lr=1e-3)
        vip._train_step(opt, X[:BATCH_SIZE], y[:BATCH_SIZE])

        # Build FTIP with identical CNN
        from src.priors.generative_functions import BayesianCNN, SimplerBayesLinear
        cnn_ftip = BayesianCNN(
            num_samples=NUM_SAMPLES, input_dim=MNIST_INPUT_DIM,
            output_dim=NUM_CLASSES, layer_model=SimplerBayesLinear,
            head_dims=[32], device=DEVICE, dtype=DTYPE, seed=SEED,
        )
        flow = CouplingFlow(2, NUM_SAMPLES * NUM_CLASSES, DEVICE, DTYPE, SEED)
        ftip = FTIP(cnn_ftip, NUM_SAMPLES, NUM_CLASSES, flow,
                    "multiclass", NUM_DATA, NUM_SAMPLES,
                    num_classes=NUM_CLASSES, device=DEVICE, dtype=DTYPE, seed=SEED)
    ...
    def test_warm_start_copies_weights(self, cnn_multiclass, mnist_multiclass_data):
        ...
        for p_ftip, p_vip in zip(ftip.generative_function.parameters(),
                                 vip.generative_function.parameters()):
