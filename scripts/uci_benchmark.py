...
Runs the UCI regression datasets with VIP, FTIP, MFVI, FBNN, or TFSVI.
...
from src.priors.generative_functions import BayesianNN, BayesLinear, SimplerBayesLinear, GP
...
# cold-start SimplerBayesLinear / alpha=1.0 sweep recorded under
# results/uci/ftip_*_alpha1.0_simpler_*. Used only when the user does
...
def parse_args():
    p = argparse.ArgumentParser(
        description="UCI regression benchmark for VIP / FTIP",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    # --- Experiment ---
    p.add_argument("--model", type=str, required=True,
                    choices=["vip", "ftip", "fbnn", "tfsvi", "mfvi"],
                    help="Model to train.")
    ...
    p.add_argument("--layer_model", type=str, default="SimplerBayesLinear",
    ...
    p.add_argument("--use_prior_regularizer", action="store_true", default=True,
...
def _build_result(dataset_name, model_type, model, args, train_loader,
    ...
    prior_stats = (
        ...
        if model_type not in ("fbnn", "tfsvi", "mfvi")
...
def main():
    ...
    for ds in datasets:
        ...
        if not args._iters_user_supplied:
            args.iterations = DEFAULT_UCI_ITERS.get(ds, 30_000)
        ...
        if args.model == "mfvi" and not args._bb_alpha_user_supplied:
            args.bb_alpha = 0.5

        # Check if results already exist
        alpha_tag = f"_alpha{args.bb_alpha}"
        layer_tag = _layer_tag(args)
        flow_tag = _flow_tag(args, args.model)
        out_path = os.path.join(
            args.output_dir,
            f"{args.model}_{ds}{alpha_tag}{layer_tag}{flow_tag}_seed{args.seed}.json",
        )
