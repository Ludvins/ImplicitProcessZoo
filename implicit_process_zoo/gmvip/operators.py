...
class EmpiricalCovarianceMatheronOperator(BaseMatheronOperator):
    ...
    def compute_cross_covariance(
        self, X: torch.Tensor, bank_Z: torch.Tensor | None = None, mu_Z: torch.Tensor | None = None
    ) -> torch.Tensor:
        X = X.to(dtype=self.Z.dtype, device=self.Z.device)
...
class RBFCardinalMatheronOperator(BaseMatheronOperator):
    ...
    def __init__(
        ...
        if self.requested_inducing_scale not in {
            "prior_cholesky",
            "rbf_cholesky",
            "prior_diag",
            "identity",
            ...
            raise ValueError(
                "inducing_scale must be 'prior_cholesky', 'rbf_cholesky', 'prior_diag', or 'identity'."
            )
        ...
        moment_Z, moment_mean, prior_K_ZZ_raw, prior_K_ZZ, prior_L_ZZ = (
            self._compute_prior_Z_moments()
        )
    ...
    def _inducing_scale_matrix(self) -> torch.Tensor:
        ...
        if self.inducing_scale == "prior_diag":
            diag = (
                torch.diagonal(prior_K_ZZ, dim1=-2, dim2=-1)
                .clamp_min(torch.finfo(self.Z.dtype).eps)
                .sqrt()
            )
            if diag.ndim == 1:
    ...
    def mean_at(self, X: torch.Tensor) -> torch.Tensor:
        ...
        if self.mean_mode == "zero":
            if self.mu_Z.ndim == 2:
                return torch.zeros(
                    X.shape[0], self.mu_Z.shape[-1], dtype=self.Z.dtype, device=self.Z.device
                )
