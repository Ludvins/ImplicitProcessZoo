...
def predict_mean_and_var_multiclass(Fmu, Fvar, num_classes, epsilon, num_gh_points, dtype, device):
    """Compute class probabilities via prob_is_largest for each class."""
    K1 = epsilon / (num_classes - 1)
    possible_outputs = [torch.full([Fmu.size()[0], 1], i) for i in range(num_classes)]
    ps = []
    for po in possible_outputs:
        p = prob_is_largest(po, Fmu, Fvar, num_classes, num_gh_points, dtype, device)
...
def gaussian_variational_expectations_full_cov(Fmu, Fcov, Y, alpha, log_variance):
    ...
    if alpha == 0:
        ...
        logpdf = (
            -0.5 * _LOG2PI - 0.5 * log_variance - 0.5 * ((Y - Fmu).square() + Fvar_diag) / sigma2
        )
        return logpdf  # (B, D)
    ...
    mvn = torch.distributions.MultivariateNormal(Fmu, covariance_matrix=cov_total + jitter)
    log_marg = mvn.log_prob(Y)  # (B,)
    const = (
        ((1 - alpha) * D / 2.0) * _LOG2PI
...
def bernoulli_variational_expectations(Fmu, Fvar, Y, num_gh_points, dtype, device):
...
def multiclass_variational_expectations(
    Fmu, Fvar, Y, num_classes, epsilon, num_gh_points, dtype, device
):
    """prob_is_largest then p*log(1-eps) + (1-p)*log(K1)."""
    K1 = epsilon / (num_classes - 1)
    p = prob_is_largest(Y, Fmu, Fvar, num_classes, num_gh_points, dtype, device)
    ve = p * torch.log(torch.tensor(1.0 - epsilon)) + (1.0 - p) * torch.log(torch.tensor(K1))
...
def compute_gp_log_marginal(m_star, phi, y, sigma2):
    ...
    sigma2_t = (
        sigma2 if torch.is_tensor(sigma2) else torch.as_tensor(sigma2, dtype=dtype, device=device)
    )
