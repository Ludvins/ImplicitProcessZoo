...
class TFSVI(nn.Module):
    ...
    def __init__(
        ...
        self._num_layers = len(self.base_net.layers) if hasattr(self.base_net, "layers") else None
        ...
        from ..priors.generative_functions import GP, ExactGP

        self._vmap_compatible = not any(
            isinstance(m, (torch.nn.LSTM, torch.nn.GRU, torch.nn.RNN, ExactGP, GP))
        ...
        self.log_sigma = nn.Parameter(torch.full((total_params,), -5.0, dtype=dtype, device=device))

        # Likelihood noise (regression only)
        if likelihood == "regression":
            self.log_variance = nn.Parameter(torch.tensor(-5.0, dtype=dtype, device=device))
    ...
    def _compute_jacobian(self, X_ctx):
        ...
        * **reverse-mode** otherwise — ``K_ctx * D`` VJPs
        ...
        K_ctx = X_ctx.shape[0]
        out_dim = self.num_classes if self.likelihood_type == "multiclass" else self.output_dim
        ...
        if self._vmap_compatible:
            strategy = "forward-mode" if mu_data.numel() < K_ctx * out_dim else "reverse-mode"
        else:
        ...
        return torch.autograd.functional.jacobian(
            f_fn,
            mu_data,
            vectorize=self._vmap_compatible,
            strategy=strategy,
        )
    ...
    def nelbo(self, X_batch, y):
        ...
        if self.bb_alpha == 0:
            ...
            ve = (
                ...
                - torch.log(torch.tensor(F.shape[0], dtype=self.dtype, device=self.device))
            ) / self.bb_alpha
        ve = torch.sum(ve)
