...
class VIP(torch.nn.Module):
    """Variational Implicit Process with Gaussian basis coefficients."""
    ...
    def __init__(
        ...
        self.register_buffer(
            "_q_sqrt_buf",
            torch.zeros(
                num_regression_coeffs, num_regression_coeffs, output_dim, dtype=dtype, device=device
            ),
        )
    ...
    def _sample_posterior(self, Fmean, Fvar, S, Fcov=None):
        ...
        if Fcov is not None:
            ...
            L = torch.linalg.cholesky(
                Fcov + 1e-6 * torch.eye(Fcov.shape[-1], dtype=Fcov.dtype, device=Fcov.device)
            )
    ...
    def _variational_expectations(self, Fmu, Fvar, Y):
        ...
        if self.likelihood_type == "regression":
            return gaussian_variational_expectations(Fmu, Fvar, Y, self.bb_alpha, self.log_variance)
    ...
    def nelbo(self, X, y):
        ...
        if self.use_full_cov_elbo and self.likelihood_type == "regression":
            ...
            bb_alpha = self._variational_expectations(F_mean.unsqueeze(0), F_var.unsqueeze(0), y)
            bb_alpha = torch.sum(bb_alpha)
        else:
            F_mean, F_var = self.predict_f(X)
            ...
            if self.bb_alpha == 0:
                ...
                ve = (
                    torch.logsumexp(self.bb_alpha * logpdf, axis=0) - math.log(self.num_mc_samples)
                ) / self.bb_alpha
    ...
    def forward(self, predict_at):
        ...
        if self.likelihood_type == "binary":
            mean, var = predict_mean_and_var_binary(Fmean.unsqueeze(0), Fvar.unsqueeze(0))
            return mean, var
