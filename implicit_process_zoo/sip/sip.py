...
class SIP(nn.Module):
    ...
    def __init__(
        ...
        self.min_log_variance = None if min_log_variance is None else float(min_log_variance)

        self.register_buffer("y_mean", torch.as_tensor(y_mean, dtype=dtype, device=self.device))
        self.register_buffer("y_std", torch.as_tensor(y_std, dtype=dtype, device=self.device))
    ...
    def _sample_prior_values(self, X, num_samples):
        ...
        if values.ndim != 3:
            raise ValueError(
                f"SIP prior samples must have shape [S, N, D], got {tuple(values.shape)}."
            )
    ...
    def _sample_u(self, num_samples):
        ...
        eps = self.posterior_noise_mean + torch.exp(0.5 * self.posterior_noise_log_var) * pre_noise
    ...
    def _critic_loss(self, num_samples=None):
        ...
        saturation = 0.5 * ((t_q.abs() > 10.0).double().mean() + (t_p.abs() > 10.0).double().mean())
    ...
    def _predict_mean_and_var(self, Fmu, Fvar):
        if self.likelihood_type == "regression":
            return predict_mean_and_var_regression(Fmu, Fvar, self.effective_log_variance())
        if self.likelihood_type == "binary":
